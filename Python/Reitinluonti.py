...
reitinluonti_app = Flask(__name__)

@reitinluonti_app.route('/reitinluonti/<pituus>') #luo apin osoitteeseen: http://127.0.0.1:3000/reitinluonti/<reitin pituus tähän>
def reitnluontifunc(pituus):
    #valitsee maat ja lentokentät reittiä varten
    try:
        pituus = int(pituus)
        tilakoodi = 200
        tehdyt = 0
            ...
            while cursor.rowcount == 0: #jos ei saa lentokenttää ekalla sql-komennolla, kokeilee sitä uudelleen
                ...
                for row in result:
                    if row[1] not in country_list:
                        ...
                        tehdyt = tehdyt+1
                        #lisää otetut lentokentät ja maat listoihin
        vastaus = {
            "airports": airport_list,
            "countries": country_list,
            "status": tilakoodi,
        } #tekee vastauksen json-muotoon
    except ValueError:
        tilakoodi = 400
        vastaus = {
            "status": tilakoodi,
    ...
    jsonresponse = json.dumps(vastaus) #muuttaa vastauksen virallisesti json-muotoon, jotta javascript voi lukea sitä
...
@reitinluonti_app.errorhandler(404)
def page_not_found(virhekoodi): #virheenhändläys
    vastaus = {
        "status" : "404",
...
if __name__ == '__main__': #pyörittää appia
    reitinluonti_app.run(use_reloader=True, host='127.0.0.1', port=3000)